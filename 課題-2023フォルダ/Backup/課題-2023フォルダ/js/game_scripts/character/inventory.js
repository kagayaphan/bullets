...
class Weapon{
    ...
    statsUp(lv){
        this._speed   += lv * this._speedAdd;
        this._range   += lv * this._rangeAdd;
        this._maxAmmo += Math.floor(lv * this._ammoAdd);
    }

    levelUp(){
        this.level++;
        this.statsUp(this.level);
    ...
    upgrade(){
        ...
        if(this.level === 0) {
            hud_manager.setMessage(localize.error_unlock_weapon);
            return false;
        }

        if(this.level >= g_weapon_max_level) {
            hud_manager.setMessage(localize.error_max_upgrade);
            return false;
        }
    ...
    draw(){
        ...
        // draw items
            ...
            for (const [id, weapon] of this._armory) {
                ...
                else drawStringInSpace(localize.remind_unlock_weapon, x, y, 50,150)
                x += xStep;
            }
        }
